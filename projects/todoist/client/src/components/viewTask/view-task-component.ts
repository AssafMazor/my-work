...
import { LabelComponent , eTaskAction } from "../taskEditor/labels/labels-component";
...
import { ChangeAlertComponent } from '../change-alert/change-alert-component';
import { DeleteAlertComponent , edeleteMode } from '../delete-alert/delete-alert-component';

import '../viewTask/view-task-component.scss';
const viewTaskTemplate = require('../viewTask/view-task-component.hbs');
...
export class ViewTaskComponent {
    ...
    setHtml(){
        let sentTime = moment(this.task.data.sentTime);
    ...
    onDatePickerBtnClick(e){
        ...
        new DatePickerComponent(this.task , this , false);
    }
    ...
    renderSubTaskList(){
        this.task.data.children.forEach((taskId:string) => {
    ...
    renderSubTask(subtask:ITask , $parentEl:any , level:number){
        ...
        let $el = $(`.sub-task-list .item.${subtask.data.name}`);
       
        subtask.data.children.forEach((taskId:string) => { 
            let subtask = this.tasksService.getTask(taskId);
    ...
    getParents(node:ITask){
        let parent = this.tasksService.getTask(node.data.parentId);
